...
export default function AddFrametype(){
    ...
    const handleSubmit = async()=>{
        ...
        if(!err)
        {
            ...
            formdata.append("frametypename" , getFrametype)
            formdata.append("icon",getIcon.bytes)
            formdata.append("status",getStatus)
            var config = {headers:{"content-type":"multipart/form-data"}}
            var result = await postDataAndImage('frametype/insertframetype',formdata,config)
            if(result)
            {
                Swal.fire({
                    position: 'top-center',
                    icon: 'success',
                    title: 'Frame Type Added Successfully',
    ...
    return (
        ...
        <h1 style={{textAlign:"center",letterSpacing:2,fontFamily:"cursive"}}>
          <span><img src={'/glasscart.png'} width={50} ></img></span><span> Add FrameType</span> </h1>
          <Grid container spacing={1}>
              <Grid item xs={6}>
            <TextField label='Frame Type Name' onChange={(event)=>setFrametype(event.target.value)}  variant='outlined' fullWidth  />
            </Grid>
            <Grid item xs={6}>
      ...
      <Grid item xs={12}>
        <Button onClick={()=>handleSubmit()} variant="contained" fullWidth color="primary">Add Frame Type</Button>
      </Grid>
      </Grid>
      <ToastContainer
      position="top-right"
