...
export default function AddShapes(){
    ...
    const handleSubmit = async()=>{
        ...
        if(!err)
        {
            ...
            formdata.append("shapename" , getShape)
            formdata.append("icon",getIcon.bytes)
            formdata.append("status",getStatus)
            var config = {headers:{"content-type":"multipart/form-data"}}
            var result = await postDataAndImage('shape/insertshape',formdata,config)
            if(result)
            {
                Swal.fire({
                    position: 'top-center',
    ...
    return (
        ...
        <h1 style={{textAlign:"center",letterSpacing:2,fontFamily:"cursive"}}>
          <span><img src={'/glasscart.png'} width={50} ></img></span><span> Add Shapes</span> </h1>
          <Grid container spacing={1}>
              <Grid item xs={6}>
            <TextField label='Shape Name' onChange={(event)=>setShape(event.target.value)}  variant='outlined' fullWidth  />
            </Grid>
            <Grid item xs={6}>
      ...
      <Grid item xs={12}>
        <Button onClick={()=>handleSubmit()} variant="contained" fullWidth color="primary">Add Shape</Button>
      </Grid>
      </Grid>
      <ToastContainer
      position="top-right"
